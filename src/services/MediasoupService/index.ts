...
import {ClientRouterEvents, ClientRouterPayloads, Router, ServerRouterEvents, ServerRouterPayloads} from "../../types";
...
class MediasoupService {
  ...
  constructor(serverConnection: ITeckosClient, router: Router, config: MediasoupConfiguration) {
    ...
    this.serverConnection.on(ServerRouterEvents.ServeStage, (payload: ServerRouterPayloads.ServeStage) => {
      const {stage, type, kind} = payload;
      if (type === "mediasoup") {
        trace("API told me to serve mediasoup for " + kind + " at stage " + payload.stage.name);
    ...
    this.serverConnection.on(ServerRouterEvents.UnServeStage, (payload: ServerRouterPayloads.UnServeStage) => {
      if (payload.type === "mediasoup") {
        trace("API told me to stop serving " + payload.kind + " the mediasoup stage " + payload.stageId);
