...
Napi::Object OvServerWrapper::Init(Napi::Env env, Napi::Object exports)
{
  ...
  Napi::Function func = DefineClass(
      env, "OvServerWrapper", {InstanceMethod("stop", &OvServerWrapper::Stop)});
    ...
    : Napi::ObjectWrap<OvServerWrapper>(info)
{
  ...
  if(length != 3) {
    Napi::TypeError::New(env, "Three arguments expected")
        .ThrowAsJavaScriptException();
  }

  if(!info[0].IsNumber()) {
    Napi::TypeError::New(env, "First argument is not a number")
        .ThrowAsJavaScriptException();
  }
  if(!info[1].IsNumber()) {
